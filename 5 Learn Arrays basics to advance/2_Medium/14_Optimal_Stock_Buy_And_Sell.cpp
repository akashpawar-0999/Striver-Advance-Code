...
public:
    int stockBuySell(vector<int> arr, int n){
        ...
        for (int i = 0 ; i < n ; i++) {                     // iteration over i=0 to i=n
            if(arr[i] < minPrice) {minPrice=arr[i] ;}       // update minPrice by every element (buy day)
            else {maxProfit = max(maxProfit,arr[i] - minPrice) ;}       // when no new array greater then minPrice , performs else
        }
